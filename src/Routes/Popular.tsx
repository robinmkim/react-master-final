...
import { IAPIResponse, getMovie, getPopular, makeImagePath } from "../api";
...
function Popular() {
  ...
  const popularMatch: PathMatch<string> | null = useMatch("/movies/:id");
  const clickedMovie =
    popularMatch?.params.id &&
    data?.results.find((movie) => movie.id === +popularMatch.params.id!);

  const movieDetail = clickedMovie && getMovie(`${clickedMovie.id}`);
